...
function sidebarForRole(role: Role | null) {
  switch (role) {
    case 'IO':
      return IOSidebar;
    case 'SHO':
    ...
    case 'LEGAL_ADVISOR':
      return LegalSidebar;
    default:
      // Role not loaded yet (or request failed) — fall back to Legal
      // sidebar rather than rendering nothing, matching the /cases list page.
      return LegalSidebar;
  }
}

function PageShell({ role, children }: { role: Role | null; children: React.ReactNode }) {
  const Sidebar = sidebarForRole(role);
