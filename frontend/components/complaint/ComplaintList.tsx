...
export default function ComplaintList({
    ...
    return (
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-xl font-semibold text-slate-900">
          {t("noComplaintsRegistered", "complaints")}
        </h2>
        <p className="mt-2 text-slate-600">
          {t("noComplaintsDescription", "complaints")}
        </p>
      </div>
    );
  }
  ...
  const sortedComplaints = [...filteredComplaints].sort((a, b) => {
    const timeA = new Date(
      a.created_at || a.incident_datetime || 0
    ).getTime();
    const timeB = new Date(
      b.created_at || b.incident_datetime || 0
    ).getTime();
    return timeB - timeA;
  });
        ...
        <thead className="bg-slate-100">
          <tr className="text-left text-sm font-semibold text-slate-700">
            <th className="px-6 py-4">
              {t("complaintTitle", "complaints") || "Complaint"}
            </th>
                            ...
                            try {
                              await axios.post(
                                ...
                                }/api/cases`,
                                {
                                  ...
                                  title:
                                    complaint.complaint_title ||
                                    `Case for ${complaint.complaint_number}`,
